img=dicomread('E:\CTLearning\s5\i000009.dcm'); 
info =dicominfo('E:\CTLearning\s5\i000009.dcm'); 
...
for ci = 256:-1:1
    ...
    if mape < 100  && bging>1500       %%1500 zhege tiaojian haixuyao zaikaolv
        for cm = 1:12%%%%%%%%%%%%%%%%%%%
            ...
            if (Lines(cm+1) - Lines(cm)) > 10 && cm > 3
                breakpoint = cm;
                break;
            end
            if beta > 1400 && cm >= breakpoint
                break;
            end
...
%             end
            ...
            imgDst(lines_i(cm),lines_j(cm)) = vaDst - 0.5*(12-cm);   %%%%%%%%
            imgDst2(lines_i(cm),lines_j(cm)) = vaDst - 0.5*(12-cm);    %%%%%%%%  
...
dicomwrite(imgDst3,'E:\CTLearning\s5\i000009_2.dcm', 'CreateMode','Copy',info);
fclose(fp);
fclose(fp2);
fclose(fpi);
fclose(fpj);
